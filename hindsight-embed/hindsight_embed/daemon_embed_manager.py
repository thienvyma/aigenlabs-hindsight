...
class DaemonEmbedManager(EmbedManager):
    ...
    def _start_daemon_locked(
        ...
        # AigenLabs Business OS may provide a bounded Codex OAuth directory for
        # the openai-codex provider. It is intentionally not persisted by the
        # HINDSIGHT_API_* profile config writer, but it must reach the daemon
        # process when supplied by the embedding app.
