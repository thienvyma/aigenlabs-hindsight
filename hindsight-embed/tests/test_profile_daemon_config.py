...
def test_codex_home_config_reaches_daemon_env(temp_home, monkeypatch):
    """Embedding apps can pass a bounded Codex OAuth home to the daemon."""
    from unittest.mock import MagicMock, patch
    ...
    with (
        ...
        manager._start_daemon(
            config={
                ...
                "CODEX_HOME": str(temp_home / "business-os-codex"),
            },
            profile="business-os",
        )

    env = captured["env"]
    assert env.get("CODEX_HOME") == str(temp_home / "business-os-codex")
